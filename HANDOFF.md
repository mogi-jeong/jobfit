...
### 포인트 보상 체계 (v0.8 변경 — 1,000P 통일)
- **기본**: 모든 공고 1,000P (`DEFAULT_POINT_REWARD`)
- **공고별 조정**: 등록·수정 폼에서 마스터/관리자가 직접 입력 (`j.point` 필드 저장)
- `pointRewardFor(j)`: `j.point` 명시 시 우선 사용, 없으면 `DEFAULT_POINT_REWARD = 1000`
- `POINT_REWARDS` 상수는 하위 호환용으로 유지하되 모든 시간대 값이 1,000P로 통일됨
...
### 4.6 주휴수당 정책 v1.1 (2026-04-28 갱신 — 동일 근무지 기준으로 일원화)

기획안에서 미정으로 남았던 "동일 근무지 vs 파트너사 통합" 안건이 v1.0에서 B안(파트너사 통합)으로 결정되었으나, 노동법 자문 후 **A안 (동일 근무지)** 로 변경됨. 이유: 주휴수당은 동일 사업장(근무지) 기준 만근에 대해서만 발생.

| 파트너사 | 만근 기준 단위 | 만근 일수 | 발생 |
|---|---|---|---|
| **컨벤션 (L타워·W힐스)** | 동일 근무지 | **주 2일 출근** | 그 근무지에서 발생 |
| **CJ대한통운** | 동일 근무지 ⭐ | **주 4일 출근** (한 곳에서) | 그 근무지에서 발생 |
| **롯데택배** | 동일 근무지 ⭐ | **주 4일 출근** (한 곳에서) | 그 근무지에서 발생 |

**공통 조건**: 주 15시간 이상 (해당 근무지 기준)

**예시 — 한 알바생의 이번 주 출근**:
```
김OO: CJ 곤지암 4일 + CJ 용인 2일 + L타워 2일
  → CJ 곤지암 4일 ✓ → 곤지암 주휴수당 발생
  → CJ 용인 2일 ✗ → 미달 (4일 필요)
  → L타워 2일 ✓ → L타워 주휴수당 발생
  → 한 알바생이 2개 근무지에서 동시 발생 가능
```

**v1.0 → v1.1 변경 영향**:
- 시드 자동 생성 한도 — 이전 "CJ 통합 4일" → "동일 근무지 4일" (한 근무지 더 자주 출근 가능)
- `validateApplicantEligibility` 경고 기준도 동일 근무지 기준으로 변경
- 알바생이 같은 파트너사 다른 근무지로는 추가 출근 가능

**구현**:
- `estimateHolidayPayForWeek(workerId, dateRefStr)` 데이터 헬퍼 (`data.js`)
- 반환: `units[]` 배열 — **모두 type='site'** (동일 근무지 기준 통일)
- 알바생 앱 팝업: 신청 시 "예상 주휴수당 약 OOO,OOO원 (참고)" 표시
- 파트너사 정산 리포트: 알바생별 주휴수당 합계 (`partnerSettlement(partnerKey, ...)`)

**지급 책임**: 알바비·주휴수당 **모두 파트너사가 직접 지급** (잡핏은 안내·집계만, 정산 책임 X)

---

### 4.7 같이하기 (Buddy / 친구 호출) — 정책 v1
...
| 7 | 포인트 합산 | 공고 포인트(`j.point`, 기본 1,000P) + 같이하기(3,000P) **별도 합산** (예: 기본 공고 + 같이하기 = 4,000P · 공고별 수동 조정 시 그 값 + 3,000P) | `pointRewardFor()` + `tryGrantBuddyBonus()` ✅ 구현 |
...
### 4.14 통근버스 시스템 — 매트릭스 모델 (v0.12 신규)

회사 통근버스(셔틀)이라 휴일·지정좌석제는 적용 안 함. 알바생은 자기 위치에서 가장 가까운 정류장을 찍어 카카오/네이버 지도 외부 길찾기로 이동.

**데이터 모델** (`busRoutesBySite[siteId]`, [data.js:53](js/data.js#L53)):
```
{
  arrival: { name, lat, lng },               // 근무지 도착지
  workStartTimes: ['HH:MM', ...],             // 작업 시작 시각 슬롯 (28개)
  routes: [
    { id, name, area, stop, lat, lng,         // 1정류장 = 1노선 (동일 좌표 다차수 = 별도 entry)
      departures: ['HH:MM', ...] }             // 작업시작시각 인덱스 매칭 출발시각
  ]
}
```

**현재 적용**: 곤지암 MegaHub만 (23노선 × 28슬롯). 다른 근무지는 시간표 수령 후 동일 모델로 추가.

**핵심 헬퍼** ([data.js:113-200](js/data.js#L113-L200)):
- `getBusSchedule(siteId)` — 근무지 통근버스 데이터 (없으면 null)
- `recommendBusForWorker(lat, lng, siteId, jobStartHHMM)` — 가장 가까운 정류장 + 매칭 출발시각
- `lookupDeparture(route, schedule, jobStartHHMM)` — 단일 셀 조회

**UI 노출 영역** (3곳):
1. 근무지 카드 — 23노선 표 + [전체 시간표 보기] 모달
2. 통근버스 길찾기(busmap) 페이지 — 카카오맵 + 정류장 핀 라벨링 + 추천 노선/도보 폴리라인
3. 알바생 앱 미리보기 통근버스 가이드 — 추천 카드 + 가까운 순 노선 리스트

**시뮬 시각** (글로벌 `globalSimState.simNow`, [app.js:11](js/app.js#L11)) — busmap·앱미리보기 양쪽 동기화. 지난 차수는 회색·취소선 처리. busmap은 추가로 작업시각 override(`simWorkStart`) 가능.

**시간표 갱신** — busmap 페이지 [📥 CSV 다운로드] / [📤 시간표 업로드] 버튼:
- XLSX 우선 (SheetJS CDN), 실패 시 CSV로 fallback
- 헤더 형식: `노선명, 권역, 정류장, 위도, 경도, 17:00, 17:10, ...`
- 메타 행은 `# 도착지, ...`로 시작 (파서가 스킵)
- 업로드는 메모리 매트릭스만 갱신 (페이지 새로고침 시 시드 데이터로 복원 — 영구 저장은 Supabase 전환 후)

### 4.15 통근버스 운영 중단 대응 — 폭설·사고·기사 결근 (이월 안건 N31)

**현재 상태**: 미구현 — 운영 정책 + 데이터 모델 확장 필요

**시나리오**:
1. **폭설/태풍** — 노선 전체(또는 일부) 운휴 결정
2. **사고/지연** — 특정 차수 1회 운휴, 알바생 다른 차수로 재배치
3. **기사 결근** — 한 노선 1일 운휴
4. **차량 정비** — 일부 차수만 결행

**필요 데이터 모델** (Supabase 전환 시):
```sql
-- 노선 동적 상태
...
**필요 UI** (운영자 측):
- 통근버스 길찾기 페이지에 [⛔ 노선 운휴 토글] 버튼 (마스터/admin1)
- 운휴 사유 입력 모달 (폭설·차량점검·기타)
- 운휴 해제 시각 선택 (당일 / 무기한 / 특정 시각까지)
- 운휴 노선 신청자 자동 알림 + 차상위 노선 추천

**필요 UI** (알바생 측):
- 추천 노선이 운휴면 자동으로 차상위 노선 안내
- 운휴 사유 + 권장 대안 (택시비 지원 / 다른 노선 / 자가 출근)
- FCM 푸시: "[긴급] 사당 노선 18:30 차수 결행 — 18:40 차수로 변경 권장"

**필요 정책** (사장님 결정 대기):
- 통근버스 결행으로 알바생이 지각/결근 시 페널티 면제 자동 적용 여부
- 차상위 노선이 만석/시간 부족이면 택시비 지원 여부 (한도)
- 결행 결정 권한 — 마스터만 / admin1까지 / 현장 admin2도 (담당 노선 한정)
- 결행 알림 발송 시점 — 즉시 / 출발 30분 전 / 등록 시
- 결행 데이터 보관 기간 (통계 분석용)

**중요한 운영 시나리오**:
- 한 알바생이 추천된 차수 출발 직전(15분 내)에 결행 통보 → 다른 노선 환승 불가 → 잡핏이 어떻게 책임 질 것인가
- 야간 작업 마지막 차수 결행 → 알바생이 출근 자체 불가 → 자동 취소 + 페널티 면제 + 보상 포인트?
...
- **N31** 통근버스 운영 중단 대응 (폭설·사고·기사 결근) — 데이터 모델·UI·정책 모두 미구현. §4.15 참조
