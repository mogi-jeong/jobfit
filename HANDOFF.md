...
### 4.6 주휴수당 정책 v1 (기획안 §7-4 N24 결정)

기획안에서 미정으로 남았던 "동일 근무지 vs 파트너사 통합" 안건이 **B안 (파트너사 통합)** 으로 결정됨.

| 파트너사 | 만근 기준 단위 | 만근 일수 | 발생 |
|---|---|---|---|
| **컨벤션 (L타워·W힐스)** | 동일 근무지 | **주 2일 출근** | 그 근무지에서 발생 |
| **CJ대한통운** | 파트너사 통합 | **주 4일 만근** (어느 근무지든) | CJ에서 발생 ⭐ |
| **롯데택배** | 파트너사 통합 | **주 4일 만근** (어느 근무지든) | 롯데에서 발생 ⭐ |

**공통 조건**: 주 15시간 이상 (해당 단위 기준)

**예시 — 한 알바생의 이번 주 출근**:
```
김OO: CJ 곤지암 2일 + CJ 용인 2일 + L타워 2일 + W힐스 1일
  → CJ 통합 4일 ✓ → CJ 주휴수당 발생 (CJ 평균 일급)
  → L타워 2일 ✓ → L타워 주휴수당 발생 (L타워 일급)
  → W힐스 1일 ✗ → 미달
  → 한 알바생이 동시에 2개 단위에서 주휴수당 발생 (각자 별도)
```

**구현**:
- `estimateHolidayPayForWeek(workerId, dateRefStr)` 데이터 헬퍼 (`data.js`)
- 반환: `units[]` 배열 — 각 단위별 자격/추정 금액 + 합계
