...
### 주휴수당 정책 v1 (기획안 §7-4 N24 결정)
- **만근 기준 단위**:
  - **컨벤션 (L타워·W힐스)**: 동일 근무지 주 2일 출근
  - **CJ대한통운**: 파트너사 통합 4일 (어느 근무지든) ⭐ B안
  - **롯데택배**: 파트너사 통합 4일 (어느 근무지든) ⭐ B안
- **추가 조건**: 주 15시간 이상 (한 단위 기준)
- **팝업 안내**: 신청 시 "이번 주 N회 출근 / X회 만근 시 주휴수당 대상" + 예상 금액 (참고용)
- **지급 책임**: 알바비/주휴수당 모두 파트너사가 직접 지급 (잡핏은 안내·집계만)
- **계산 헬퍼**: `estimateHolidayPayForWeek(workerId, dateStr)` → `units[]` 단위별 자격 분석
